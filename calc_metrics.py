...
def calc_accuracy(pred_data, num_thresh=1000):
    ...
    auc = calculate_auc(conf_mat_thresh)
    print(auc)
    ...
    for fp_req in fps_req:
        for i in range(num_thresh):
            ...
            if f/num_images < fp_req:                
                ...
                print(fp_req, sensitivity[i-1], sensitivity[i], thresholds[i], f/num_images, conf_mat_thresh[i])
                break
    return senses_req, fps_req, sensitivity, specificity
    

def save_boxes(img_path, folder_path, boxes, labels, scores=None, flag = True):
    ...
    for i,box in enumerate(boxes):
        ...
        if(labels[i]=="pred" and flag==False):
            cv2.rectangle(img, (x, y), (x + w, y + h), (0, 0, 255), 10)
        elif(labels[i]=="pred" and flag==True):
...
def save_cases(cases_list, pred_list, save_folder, thresh = 0.1041041041041041, flag = True):
    ...
    for i,item in enumerate(items):
        ...
        save_boxes(item["img_path"], case_folder, boxes, box_labels, scores = item["pred"]['scores'][select_mask], flag=flag)
...
def concatenate_images(image_paths, output_path):
    ...
    half = 0.25
    new_image = new_image.resize( [int(half * s) for s in new_image.size] )
...
def join_cases(cases_list, src_path):
    ...
    for i,case in enumerate(cases_list):
        ...
        images_attn = glob.glob(os.path.join(src_path,"max",str(case))+"/*.png"); images_attn = sort_images(images_attn)
...
def get_error_boxes(pred_data, save_folder, thresh = 0.1313131313, flag = True):
    os.makedirs(save_folder, exist_ok=True)
    ...
    for i, item in enumerate(tqdm(selected_pred)):
        ...
        if(len(["gt" for item2 in item["target"]['boxes']])>0):
            ...
            save_boxes(item["img_path"], folder_path, boxes, box_labels, scores = item["pred"]['scores'][select_mask], flag = flag)
        # gt_data = item['target']
        # pred = item['pred']
        # scores = pred['scores']
